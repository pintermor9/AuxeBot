...
class Fun(commands.Cog):
    ...
    @commands.command(
        name="8ball",
        aliases=["eightball"],
        description="Gives you a random answer to your question.",
    )
    ...
    @commands.command(
        aliases=["noice", "nájsz", "nojsz", "nolysz"], description="Sends a nice gif."
    )
    ...
    @commands.command(
        aliases=["itsthisforthat", "this/that", "thisthat"],
        description="itsthisforthat.com",
    )
    async def thisforthat(self, ctx):
        r = orjson.loads(
            await self.bot.api.get(
                "http://itsthisforthat.com/api.php?json", use_base=False
            )
        )
